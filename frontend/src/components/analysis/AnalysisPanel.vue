...
type Tab = { id: typeof analysis.activeTab; label: string }
const tabs: Tab[] = [
  { id: 'summary', label: '代码解读' },
  { id: 'bug',     label: 'Bug 检测' },
  { id: 'deps',    label: '依赖图' },
  { id: 'history', label: '历史' },
]
      ...
      <div class="px-4 py-3 flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span class="font-mono text-[9px] font-bold px-1.5 py-0.5 rounded
                       bg-cyan-dim text-cyan border border-cyan/20">LLM</span>
        </div>
        <!-- Abort button -->
        <button
          v-if="analysis.streaming"
          class="font-mono text-[10px] text-red-accent hover:text-red-accent/70 transition-colors"
...
.analysis-tab {
  @apply font-mono text-[11px] px-3 py-2 border-b-2 border-transparent
