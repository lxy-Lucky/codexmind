...
<template>
  <div class="flex items-center justify-between px-5 py-2
    ...
    <div v-else class="font-mono text-[11px] text-text-muted">未打开文件</div>

    <!-- Toolbar actions -->
    <div class="flex items-center gap-1.5">
      <span v-if="editor.currentFile" class="font-mono text-[10px] text-text-muted mr-2">
        {{ editor.currentFile.line_count }} 行 · {{ editor.currentFile.language }}
      </span>

      <button class="toolbar-btn" title="代码解读" @click="triggerAnalysis('summary')">
        ◆ 解读
      </button>
